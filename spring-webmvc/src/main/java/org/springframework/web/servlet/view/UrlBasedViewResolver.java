 ...
 *
 * {@link org.springframework.web.servlet.ViewResolver}接口的简单实现，允许将符号视图名称直接解析为URL，
 * 而无需显式映射定义。 如果您的符号名称以直接的方式与视图资源的名称匹配（即符号名称是资源文件名的唯一部分），
 * 而无需为每个视图定义专用映射，则这将很有用。
 *
 ...
 *
 * 支持{@link AbstractUrlBasedView}子类，例如{@link InternalResourceView}和{@link org.springframework.web.servlet.view.freemarker.FreeMarkerView}。
 ...
 *
 * 视图名称本身可以是资源URL，也可以通过指定的前缀和/或后缀进行扩充。 明确支持将保存RequestContext的属性导出到所有视图。
 *
 ...
 *
 * 作为一项特殊功能，可以通过“ redirect：”前缀指定重定向URL。
 ...
 *
 * 此外，可以通过“ forward：”前缀指定转发URL。
 *   例如：“ forward：myAction”将触发对给定URL的转发，而不是解析为标准视图名称。
 *   通常用于控制器URL。 它不应该用于JSP URL-在此处使用逻辑视图名称
 *
 * <p>Note: This class does not support localized resolution, i.e. resolving
 * a symbolic view name to different resources depending on the current locale.
 *
 * 注意：此类不支持本地化的解析，即根据当前语言环境将符号视图名称解析为不同的资源。
 *
 ...
 *
 * 注意：链接ViewResolvers时，UrlBasedViewResolver将检查{@linkplain AbstractUrlBasedView＃checkResource指定的资源是否确实存在}。
 * 但是，使用{@link InternalResourceView}，通常不可能预先确定目标资源的存在。
 * 在这种情况下，UrlBasedViewResolver将始终为任何给定视图名称返回一个View。
